...
import { activityByGame, countBy, countSeries, distinctSeries, lagPercentiles, weekdayHourGrid } from "../src/lib/events.ts";
...
test("activityByGame aggregates the lifecycle events per game", () => {
  const events = [
    ...
    event("quick_play_started", "2026-09-17T11:30:00Z", { game_id: "glyph" }),
  ];
  const [sudoku, glyph] = activityByGame(events);
